...
int main(void) { //메인함수 시작
    ...
    VideoCapture src("7_lt_ccw_100rpm_in.mp4"); // mp4 파일로부터 비디오를 읽기 위한 VideoCapture 객체를 생성하고 초기화
    if (!src.isOpened()) {  //src가 열리지 않는다면
        cerr << "비디오 오류" << endl; //문자열 출력
    ...
    d = src.get(CAP_PROP_FRAME_WIDTH) / 2; //d에 영상의 x의 중앙값(320)을 저장

    double prevLineCenter = d; //이전 라인 x값을 저장할 변수 선언 및 d값을 저장
    bool isFirstFrame = true;  //처음 영상을 설정하기 위한 bool변수, true로 설정

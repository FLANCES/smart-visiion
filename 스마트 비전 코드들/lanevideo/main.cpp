...
int main(void) {
    ...
    VideoCapture src("lanefollow_100rpm_cw.mp4");
    if (!src.isOpened()) {
        cerr << "비디오 오류" << endl;
    ...
    int front = 100; //직진속도 100으로 설정
    double gain = 0.4; //gain값을 0.4로 설정
    ...
    while (1) { //무한반복
        ...
        for (int i = 1; i < left_cnt; i++) { //왼쪽 라인 검출 반복문
            ...
            if (left_minDistance == left_distanceToCurrentLine && left_distanceToCurrentLine < 50) { //left_minDistance == left_distanceToCurrentLine && left_distanceToCurrentLine < 50이면
        ...
        for (int i = 1; i < right_cnt; i++) { //오른쪽 라인 검출 반복문
            ...
            if (right_minDistance == right_distanceToCurrentLine && right_distanceToCurrentLine < 50) { //right_minDistance == right_distanceToCurrentLine && right_distanceToCurrentLine < 50이면
